...
(function() {
  ...
  // formatDate(new Date()) "2020-03-05 16:50:21.978"
  ...
  // _.UUID()() 生成 170a9dcaaaf373-00e97bc8d6a14a-3b65410e-1767681-170a9dcaab089c
  _.UUID = (function() {
    var T = function() {
      var d = 1 * new Date(),
        i = 0;
  ...
  _.getSourceFromReferrer = function() {
    ...
    var search_engine = collect.para.source_type.search; // 例如 百度，谷歌，搜搜
    var social_engine = collect.para.source_type.social; // 例如 人人，微博
...
var store = collect.store = {
  ...
  change: function(name, value) {  // 修改对应的name的值
  ...
  getCookieName: function() {
    ...
    if (collect.para.cross_subdomain === false) {
      ...
      sub = 'sensorsdata2015jssdkcross'; // cookie 的名称
    }
    return sub;
  },
  init: function() {
  ...
  setNotice: function(web_url) {
    ...
    if (web_url && web_url[0] && web_url[1]) {
      if (web_url[1].slice(0, 5) === 'http:' && location.protocol === 'https') {
        collect.errorMsg = '您的当前页面是https的地址，分析环境也必须是https！';
      }
    }
    ...
    var delayTime = interDelay({
      ...
      func: function(para, isClose) {
        ...
        if ((delay_time > collect.para.heatmap.scroll_delay_time && offsetTop - para.$viewport_position !== 0) || isClose) {
          ...
          // collect.para.heatmap.scroll_event_duration 默认值 18ms
...
if (typeof window['sensorsDataAnalytic201505'] === 'string') {
  ...
  collect = window['sensorsDataAnalytic201505'];
}
